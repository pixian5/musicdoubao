...
VERSION = 34
HOP_LENGTH = 512
BREATH_SEGMENT_SHIFT_SEC = 0.035
...
def _shift_segments_left(segments, sr, shift_sec=BREATH_SEGMENT_SHIFT_SEC):
    if not segments or sr is None or sr <= 0:
        return segments
    shift_samples = int(round(shift_sec * sr))
    if shift_samples <= 0:
        return segments
...
def process_breath(input_path, atten_db=18, sensitivity=7, peak_reject_threshold=0.20, percentile_reject_threshold=0.20):
    try:
        ...
        breath_segments = _shift_segments_left(breath_segments, sr)
        ...
        for start, end in breath_segments:
            ...
            # 用更平滑的“下凹”包络压制吸气，减少片段边缘和内部的突兀感。
            t = np.linspace(0.0, 1.0, len(segment), dtype=np.float32)
            center_shape = np.power(np.sin(np.pi * t), 2.45)
            energy_focus = np.power(np.clip(env_norm, 0.0, 1.0), 0.58)
            attenuation_depth = (1.0 - gain) * (0.05 + 0.95 * center_shape) * (0.06 + 0.94 * energy_focus)

            fade_len = min(int(0.040 * sr), max((end - start) // 4, 1))
            edge_fade = np.ones_like(segment, dtype=np.float32)
            if fade_len > 1 and len(segment) > fade_len * 2:
                edge_curve = 0.5 - 0.5 * np.cos(np.linspace(0.0, np.pi, fade_len, dtype=np.float32))
                edge_fade[:fade_len] = edge_curve
                edge_fade[-fade_len:] = edge_curve[::-1]
            attenuation_depth *= edge_fade
            attenuation_depth = _moving_average(attenuation_depth, max(12, int(0.012 * sr)))

            envelope = 1.0 - attenuation_depth
            center_silence_floor = gain * (1.0 - 0.992 * center_shape)
            edge_floor = gain + (1.0 - gain) * (1.0 - center_shape) * 0.18
            min_envelope = np.minimum(edge_floor, np.maximum(center_silence_floor, gain * 0.01))
            envelope = np.maximum(envelope, min_envelope)
            envelope = np.clip(envelope, gain * 0.01, 1.0)
...
class BreathReducerApp:
    ...
    def run_process(self):
        ...
        try:
            peak_reject_threshold = float(self.peak_reject_var.get()) / 100.0
    ...
    def _draw_wave_envelope(self, ax, audio, title, active=False):
        ...
        try:
            peak_line = float(self.peak_reject_var.get()) / 100.0
